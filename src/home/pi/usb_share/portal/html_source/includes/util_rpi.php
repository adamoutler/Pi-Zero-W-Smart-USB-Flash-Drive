...
if (empty($_GET['a'])) {
    ...
    if($request_type == 'host_info') {
        ...
        $data['rebuilding_usb_share'] = file_exists('/home/pi/.usb_share_resources/portal/rebuilding_usb') ? 'true' : 'false';
        $data['mono_x_enabled'] = file_exists('/home/pi/.usb_share_resources/portal/enable_mono_x') ? 'true' : 'false';
        $data['camera_enabled'] = file_exists('/home/pi/.usb_share_resources/portal/enable_camera') ? 'true' : 'false';
      ...
      $cmd = "sudo ./scripts/rpi_update.py --current_hostname " . $current_hostname . " --new_hostname " . $new_hostname . " --enable_camera " . $enable_camera ." --rotation " . $rotation . " &";
      shell_exec($cmd);
    } else if ($request_type == 'update_status') {
      if(file_exists("/home/pi/.usb_share_resources/portal/updating_rpi"))
      {
          $data['status'] = file_get_contents("/home/pi/.usb_share_resources/portal/updating_rpi");
      }
      else 
      {
        $data['status'] = "stopped";
      ...
      $cmd = 'sudo ./scripts/rpi_usb_disk_create.sh ' . $new_usb_size . '&';
      shell_exec($cmd);
    } else if ($request_type == 'rebuild_status') {
      if(file_exists("/home/pi/.usb_share_resources/portal/rebuilding_usb"))
      {
          $data['status'] = file_get_contents("/home/pi/.usb_share_resources/portal/rebuilding_usb");
      }
      else 
      {
        $data['status'] = "stopped";
      ...
      $current_usb_size = shell_exec('./scripts/rpi_usb_disk_mgmt.py -umb');
      $data['disk_size'] = $current_usb_size;
    } else if ($request_type == 'rebuild_reboot') {
      $cmd = 'sudo ./scripts/rpi_usb_disk_create.sh reboot';
      shell_exec($cmd); 
    } else if ($request_type == 'reboot') {
      $cmd = "sudo ./scripts/rpi_update.py --reboot yes;";
      shell_exec($cmd); 
