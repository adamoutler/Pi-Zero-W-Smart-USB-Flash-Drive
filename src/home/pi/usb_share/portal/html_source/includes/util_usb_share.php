...
if (empty($_GET['a'])) {
    ...
    if($request_type == 'disk_stats') {
        ...
        $avail_disk_space = (float) shell_exec('./scripts/rpi_usb_disk_mgmt.py -a');
        $max_usb_share = floor(($disk_size / 1024) + $avail_disk_space - 2);
        ...
        $cmd = "./scripts/rpi_usb_control.py --action 'reset';";
        shell_exec($cmd);
    } else if ($request_type == 'usb_unplug') {
        $cmd = "./scripts/rpi_usb_control.py --action 'unplug';";
        shell_exec($cmd);
    } else if ($request_type == 'usb_reset_status') {
        if(file_exists("/home/pi/.usb_share_resources/portal/usb_reset_status"))
        {
            $data['status'] = file_get_contents("/home/pi/.usb_share_resources/portal/usb_reset_status");
        }
        else 
        {
          $data['status'] = "stopped";
